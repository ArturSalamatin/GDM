...
std::vector<std::vector<WellDataHandler::DataReader::ValueContainer>> WellDataHandler::DataReader::Read(std::wstring path)
{
	if (std::filesystem::exists(path))
	{
		std::vector<std::vector<WellDataHandler::DataReader::ValueContainer>> out;
		...
		return std::vector<std::vector<WellDataHandler::DataReader::ValueContainer>>();
	}
}
/// <summary>
/// Ключ:Значение
...
void WellDataHandler::PerfData::Push(std::wstring path, std::wstring name)
{
	...
	for (const auto& line : data)
	{
		...
		ldata[L"time"] = line[0].Value;
		ldata[L"is_open"] = (line[13].Value == 1) ? 1 : 0;
		ldata[L"perf_start"] = line[6].Value;
		ldata[L"perf_stop"] = line[7].Value;
		ldata[L"perf_density"] = line[12].Value;
...
void WellDataHandler::PerfData::Push(std::wstring path_to_perf)
{
	...
	std::unique_ptr<std::unique_ptr<std::unique_ptr<wchar_t[]>[]>[]> data = std::move(parser.ReadA(size));
	...
	for (int i = 1; i < size; i++)
	{
		...
		/*std::regex rg("\\D+");
		std::smatch sm;
		if (std::regex_search(name, sm, rg))
		{
			std::string match = sm[0].str();
			if ((unsigned char)match[0] < 0x7F)
			{
				std::cout << match;
			}
		}*/
		...
		std::map<std::wstring, std::wstring> sldata;
		sldata[L"type"] = std::wstring(line[4].get());
		StringContainer[name].push_back(sldata);
...
void WellDataHandler::GISData::Push(std::wstring path, std::wstring name)
{
}
...
void WellDataHandler::GISData::Push(std::wstring path_to_gis_file)
{
	...
	for (int i = 1; i < size; i++)
	{
			...
			)
		{
			std::wstring name(line[0].get());
			std::map<std::wstring, float> ldata;
			ldata[L"layer_start"] = wcstof(DataReader::ReplaceCommaWithDot(line[3].get()).c_str(), &stopscan);
			ldata[L"layer_stop"] = wcstof(DataReader::ReplaceCommaWithDot(line[5].get()).c_str(), &stopscan);
			ldata[L"layer_height"] = wcstof(DataReader::ReplaceCommaWithDot(line[4].get()).c_str(), &stopscan);
			...
			lsdata[L"in_layer_name"] = std::wstring(line[2].get());
			StringContainer[name].push_back(lsdata);
		}
		line.reset();
	}
	data.reset();
...
void WellDataHandler::MerData::Push(std::wstring path, std::wstring name)
{
	...
	for (const auto& line : data)
	{
		std::map<std::wstring, float> ldata;
		ldata[L"time"] = line[0].Value;
		ldata[L"oil"] = line[9].Value;
		ldata[L"water"] = line[10].Value;
...
//#pragma omp parallel for
	for (int i = 1; i < size; i++)
	{
		...
		std::wstring name(line[0].get());
		std::map<std::wstring, float> ldata;
	//	std::map<std::wstring, std::wstring> sdata;
	//	sdata[L"layers"] = line[2].get();
	//	ldata[L"time"] = PathUtils::Utils::ConvertDateToExcelDate(std::wstring(line[1].get()));
		...
		// if type == 0
		double oil_v = 0.0, oil_m = 0.0, water_v = 0.0, water_m = 0.0;
		// else
		if (type == 1)
		{
			oil_v = wcstof(DataReader::ReplaceCommaWithDot(line[16].get()).c_str(), &stopstring); // stof(line[17]);
			water_v = wcstof(DataReader::ReplaceCommaWithDot(line[17].get()).c_str(), &stopstring); // stof(line[18]);
			oil_m = wcstof(DataReader::ReplaceCommaWithDot(line[10].get()).c_str(), &stopstring) * 1000; // stof(line[17]);
			water_m = wcstof(DataReader::ReplaceCommaWithDot(line[11].get()).c_str(), &stopstring) * 1000; // stof(line[18]);
		}
		else if (type == -1)
		{
			oil_v = 0.0;
			water_v = -wcstof(DataReader::ReplaceCommaWithDot(line[21].get()).c_str(), &stopstring); // -stof(line[21]);
			oil_m = 0.0;
			water_m = -wcstof(DataReader::ReplaceCommaWithDot(line[21].get()).c_str(), &stopstring)*1000; // -stof(line[21]);
		}

		Container[name].emplace_back(std::initializer_list<std::pair<const std::wstring, float>> {
			{L"time", PathUtils::Utils::ConvertDateToExcelDate(line[1].get())},
			{ L"oil_v",  oil_v },
			{ L"water_v", water_v },
			{ L"oil_m",  oil_m },
			{ L"water_m", water_m },
	...
	//	Container[name].push_back(std::move(ldata));
	//	StringContainer[name].push_back(std::move(sdata));
	}
	data.reset();
	Optimize();
}
	...
	std::map<std::wstring, float> Data)
{
	...
	CumWater[Name] += (Data[L"water"] > 0) ? Data[L"water"] : 0;
	CumPumpWater[Name] += (Data[L"pump_water"] > 0) ? Data[L"pump_water"] : 0;

	SumPumpTime[Name] += (
		!Data[L"type"]
		&&
		Data[L"is_work"]) ? Data[L"worked_time"] / 730 : 0;

	SumIdleTime[Name] += Data[L"idle_time"] / 730;
	...
	SumMixedTime[Name] += (
		...
		Data[L"type"]
		&&
		wtrc >= 0.2) ? Data[L"worked_time"] / 730 : 0;
...
float WellDataHandler::MerData::GetValue(int id, std::wstring name)
{
	...
	case 0:
		...
		break;
	case 1:
		return CumWater[name];
		break;
	case 2:
		return CumPumpWater[name];
		break;
	case 3:
		return CumDryOil[name];
		break;
	case 4:
		return SumPumpTime[name];
		break;
	case 5:
		return SumIdleTime[name];
		break;
	case 6:
		return SumMixedTime[name];
		break;
	case 7:
		return SumDryTime[name];
		break;
	default:
		return 0;
		break;
...
void WellDataHandler::GDISData::Push(std::wstring path, std::wstring name)
{
...
void WellDataHandler::GDISData::Push(std::wstring path_to_gdis)
{
	...
	for (int i = 1; i < size; i++)
	{
		...
		if (/*line.size() > 85 &&*/ !wcscmp(line[48].get(), L"Рпл") || !wcscmp(line[48].get(), L"Нст"))
		{
			std::wstring name(line[0].get());
			std::map<std::wstring, float> ldata;
			ldata[L"time"] = PathUtils::Utils::ConvertDateToExcelDate(std::wstring(line[50].get()));
			ldata[L"pressure"] = wcstof(DataReader::ReplaceCommaWithDot(line[68].get()).c_str(), &stopscan);
			Container[name].push_back(ldata);
		}
		line.reset();
	}
...
std::map<std::wstring, std::vector<std::tuple<int, int, int, std::pair<float, float>>>> WellDataHandler::DataHandleUtils::ComparePerfWithGIS(GISData& gisd, PerfData& perfd, std::wstring name)
{
	...
	{
		float pstart = perf_dat[i][L"perf_start"];
		float pstop = perf_dat[i][L"perf_stop"];
...
void WellDataHandler::DataHandleUtils::CompareGeoChemWithWaterCut(WCData& wcdat, GeoChemData& gcd)
{
	...
	//	auto wc_well_data = wcdat.GetDataPerWell(gc_well);
	//	for (const auto& _gc_well_data : gc_well_data)
	//	{
	//		int ctime = _gc_well_data.at(L"time");

	//		int closest_index = -1;
	//		float last_diffs = 1e36;
	...
	//		if (closest_index != -1)
	//		{
	//			obv = wc_well_data[closest_index].at(L"water_cut");
	//			current_obv = wc_well_data[closest_index].at(L"water_cut_current");
	//			pl_tek = wc_well_data[closest_index].at(L"pl_tek");
	//		}
	//		toWrite << _gc_well_data.at(L"p/p") << ";"
	//			<< gc_well << ";"
	//			<< _gc_well_data.at(L"part_number") << ";"
	//			<< _gc_well_data.at(L"time") << ";"
	//			<< _gc_well_data.at(L"methane") << ";"
	//			<< _gc_well_data.at(L"ethane") << ";"
	//			<< _gc_well_data.at(L"propane") << ";"
	//			<< _gc_well_data.at(L"ibutane") << ";"
	//			<< _gc_well_data.at(L"butane") << ";"
	//			<< _gc_well_data.at(L"ipentane") << ";"
	//			<< _gc_well_data.at(L"pentane") << ";"
	//			<< _gc_well_data.at(L"gexane") << ";"
	//			<< _gc_well_data.at(L"ibut/but") << ";"
	//			<< _gc_well_data.at(L"ipen/pen") << ";"
	//			<< _gc_well_data.at(L"ml_isobutane") << ";"
	//			<< _gc_well_data.at(L"ml_butane") << ";"
	//			<< _gc_well_data.at(L"ml_isobutane_butane") << ";"
	//			<< std::to_string(obv) << ";"
	//			<< std::to_string(current_obv) << ";"
	//			<< std::to_string(pl_tek) << "\n";
	//	}
	//}
	//toWrite.close();
}
...
std::map<std::wstring, std::vector<std::map<std::wstring, float>>> WellDataHandler::DataHandleUtils::DivideProduction(
	std::wstring name,
	MerData& mer,
	GISData& gis,
	PerfData& perf,
	std::vector<std::wstring> LayersName)
{
	...
	for (const auto& mr : mers)
	{
		...
		{
			parst[i] = (summ_kh) ? parst[i] / summ_kh : 0;
		}
		...
		for (int i = 0; i < parst.size(); i++)
		{
			...
			ldata[L"oil"] = oil;
			ldata[L"water"] = water;
			ldata[L"pump_water"] = pwater;
			ldata[L"time"] = mr.at(L"time");
			ldata[L"worked_time"] = mr.at(L"worked_time");
			ldata[L"idle_time"] = mr.at(L"idle_time");
			ldata[L"type"] = mr.at(L"type");
			ldata[L"is_work"] = mr.at(L"is_work");
			ldata[L"colection_time"] = mr.at(L"colection_time");
			...
			for (const auto& ln : LayersName)
			{
				...
				if (elayer[i].find(prefix) != std::string::npos)
				{
					...
					if (suffix.find(elayer_suffix) != std::wstring::npos)
					{
						...
						{
							(*(out[ln].end() - 1)).at(L"oil") += oil;
							(*(out[ln].end() - 1)).at(L"water") += water;
							(*(out[ln].end() - 1)).at(L"pump_water") += pwater;
						}
						else {
							out[ln].push_back(ldata);
						}
					}
				}
			}
...
void WellDataHandler::WCData::Push(std::wstring path, std::wstring name)
{
}
...
void WellDataHandler::WCData::Push(std::wstring path_to_wc)
{
	...
	for (const auto& line : data)
	{
		std::wstring name = line[1];
		std::map<std::wstring, float> ldata;
		ldata[L"time"] = PathUtils::Utils::ConvertDateToExcelDate(line[9]);
		ldata[L"water_cut"] = stof(line[12]);
		ldata[L"water_cut_current"] = stof(line[16]);
		ldata[L"pl_tek"] = stof(line[17]);
		Container[name].push_back(ldata);
	}
	Optimize();
}

void WellDataHandler::GeoChemData::Push(std::wstring path, std::wstring name)
{
}
...
void WellDataHandler::GeoChemData::Push(std::wstring path_to_gc)
{
	...
	for (const auto& line : data)
	{
		std::wstring name = line[1];
		std::map<std::wstring, float> ldata;
		ldata[L"p/p"] = stof(line[0]);
		ldata[L"part_number"] = stof(line[2]);
		ldata[L"time"] = PathUtils::Utils::ConvertDateToExcelDate(line[3]);
		ldata[L"methane"] = stof(line[4]);
		ldata[L"ethane"] = stof(line[5]);
		ldata[L"propane"] = stof(line[6]);
		ldata[L"ibutane"] = stof(line[7]);
		ldata[L"butane"] = stof(line[8]);
		ldata[L"ipentane"] = stof(line[9]);
		ldata[L"pentane"] = stof(line[10]);
		ldata[L"gexane"] = stof(line[11]);
		ldata[L"ibut/but"] = stof(line[12]);
		ldata[L"ipen/pen"] = stof(line[14]);
		ldata[L"ml_isobutane"] = stof(line[14]);
		ldata[L"ml_butane"] = stof(line[15]);
		ldata[L"ml_isobutane_butane"] = stof(line[16]);
		Container[name].push_back(ldata);
		WellsName[name] = 0;
	}
	Optimize();
...
void WellDataHandler::FECData::Push(std::wstring path, std::wstring name)
{
}
...
void WellDataHandler::FECData::Push(std::wstring path_to_fec)
{
	...
	for (int i = 1; i < size; i++)
	{
		...
		if (*line[5].get() != L'\x0')
		{
			std::wstring name(line[1].get());
			std::map<std::wstring, float> ldata;
			ldata[L"time"] = PathUtils::Utils::ConvertDateToExcelDate(std::wstring(line[5].get()));
			Container[name].push_back(ldata);
		}
		line.reset();
		/*	}
...
std::vector<std::wstring> WellDataHandler::FECData::GetWellsPerPeriod(int start, int stop)
{
	...
	{
		for (const auto& d : wd.second)
		{
			if (d.at(L"time") >= start && d.at(L"time") <= stop)
			{
				out.push_back(wd.first);
			}
		}
	}
	return out;
...
void WellDataHandler::AnomData::Push(std::wstring path, std::wstring name)
{
}
...
void WellDataHandler::KH_MerLayeredData::Push(
	MerData& mer,
	GISData& gis,
	PerfData& perf,
	std::vector<std::wstring> LayersName,
	std::vector<std::wstring> WellNames)
{
	for (const auto& ln : LayersName)
	{
		Mers.insert({ ln, std::make_shared<MerData>() });
	}
	std::atomic_int th_counter = 0;
	int ref_th_counter = 0;
	std::mutex MersMutex;
	std::vector<bool> th_status(16);

	auto lambda_divide_prod = [](
		std::wstring wn,
		std::vector<std::wstring>& _LayersName,
		std::map<std::wstring, std::shared_ptr<MerData>>& _Mers,
		std::mutex& _MersMutex,
		MerData& _mer,
		GISData& _gis,
		PerfData& _perf,
		std::atomic_int& sync,
	...
	for (auto& mer : Mers)
	{
		mer.second->ManualOptimize();
	}
}

void WellDataHandler::RaschData::Push(std::wstring path, std::wstring name)
{
}
...
void WellDataHandler::WellCoordData::Push(std::wstring path, std::wstring name)
{
}

void WellDataHandler::WellCoordData::Push(std::wstring path_to_pp)
{
	std::lock_guard<std::mutex> lock(DataAccessMutex);
	auto parser = UniversalSCParser::UTF8SVParser(path_to_pp);
	size_t size = 0;
	std::unique_ptr<std::unique_ptr<std::unique_ptr<wchar_t[]>[]>[]> data = std::move(parser.ReadA(size));
